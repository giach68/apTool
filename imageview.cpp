    ...
    ui(new Ui::ImageView)
{
    ...
    x1=0;
    y1=0;
    CoordinatesSet1=false;

    x2=0;
    y2=0;
    CoordinatesSet2=false;

    point1 = new QLabel(imageLabel);
    point2 = new QLabel(imageLabel);
    ...
    point1->setPixmap(image);
    QPen pen2(Qt::red);
    painter.setPen(pen2);
    painter.drawEllipse(rectangle);
    point2->setPixmap(image);
    point1->setGeometry(QRect(0,0,0,0));
    point2->setGeometry(QRect(0,0,0,0));
...
void ImageView::showImg(QImage img)
{
    if (img.isNull()) {
        return;
    }
    ...
    baseFactor = this->width()/(float)s.width();
...
void ImageView::setImage(cv::Mat imagev)
{

        double min, max;

    //    imageLabel->setPixmap(QPixmap::fromImage(imagev));
        imageLabel->setPixmap(QPixmap::fromImage((QImage(imagev.data,imagev.cols,imagev.rows,imagev.step,QImage::Format_RGB888))));
        s = QSize(imagev.cols,imagev.rows);  //imagev.size();
        ratio = (float)s.width()/(float)s.height();
        int r,g,b,a;
        int cx= s.width() /2;
        int cy= s.height()/2;
...
void ImageView::mousePressEvent(QMouseEvent *event)
{
    ...
    if(active==11)
        if(event->button()==Qt::LeftButton) {
            x1=imageLabel->mapFromParent(event->pos()).x();
            y1=imageLabel->mapFromParent(event->pos()).y();
            CoordinatesSet1=false;
            point1->setGeometry(QRect(0,0,0,0));
            if (x1>imageLabel->width() || y1>imageLabel->height())
                return;
            CoordinatesSet1=true;
            white1->setGeometry(QRect(0,0,0,0));
            point1->setGeometry(x1-5, y1-5, 11, 11);
        }

    if(active==12)
        if(event->button()==Qt::LeftButton) {
            x2=imageLabel->mapFromParent(event->pos()).x();
            y2=imageLabel->mapFromParent(event->pos()).y();
            CoordinatesSet2=false;
            point2->setGeometry(QRect(0,0,0,0));
            if (x2>imageLabel->width() || y2>imageLabel->height())
                return;
            CoordinatesSet2=true;
            white2->setGeometry(QRect(0,0,0,0));
            point2->setGeometry(x2-5, y2-5, 11, 11);
        }
    //fine eventi aggiunti
